...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Coaching' AND type = 'U')
BEGIN
    CREATE TABLE Coaching (
        id_coaching INT IDENTITY(1,1) PRIMARY KEY,
        Hyp VARCHAR(50) NOT NULL,
        Total_Stat DECIMAL(18,2),
        UserName VARCHAR(255),
        date_coaching DATETIME,
        Num_Bp VARCHAR(50),
        Offre VARCHAR(255),
        Sous_Motif VARCHAR(255),
        Short_Message VARCHAR(MAX),
        Country VARCHAR(100),
        City VARCHAR(100),
        Montant DECIMAL(18,2),
        Order_Date DATETIME,
        
        -- Contrainte pour s'assurer que Hyp est unique si utilisé comme clé
        CONSTRAINT UK_Coaching_Hyp UNIQUE (Hyp)
    );
    
    PRINT 'La table Coaching a été créée avec succès.';
END
ELSE
BEGIN
    PRINT 'La table Coaching existe déjà.';
END